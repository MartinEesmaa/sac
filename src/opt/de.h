...
class OptDE : public Opt {
  public:
    ...
    std::size_t eval_pop(opt_func func,span<ppoint> pop)
    {
      std::size_t n=0;
      while (n < pop.size())
      {
        ...
        std::size_t k=eval_points_mt(func,span{begin(pop)+start,begin(pop) + ende});
    ...
    virtual ppoint run(opt_func func,const vec1D &xstart) override
    {
      ...
      span<ppoint> pop_span(pop.begin()+1,pop.end());
      ...
      while (nfunc<cfg.nfunc_max)
      {
        ...
        {
          auto candidate = generate_candidate(pop, xb.second, iagent, mCR, mF);
          auto xtrial = std::get<0>(candidate);
          auto tCR = std::get<1>(candidate);
          auto tF = std::get<2>(candidate);
          gen_mut[iagent] = {tCR, tF};
          gen_pop[iagent].second = xtrial;
        }

        // evaluate trial population
        k=eval_pop(func,span(gen_pop));
        nfunc+=k;
